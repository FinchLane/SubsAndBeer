...
fun PaymentMethodScreen(
    ...
    Scaffold(
        topBar = { AppBarBack(nav = {navController.popBackStack()}, title = "Платежные методы") },
        ...
        Box(
                ...
                LazyColumn(
                    ...
                    items(paymentMethods){ paymentMethod ->
                        PaymentMethodItem(
                            ...
                            countSub = viewModel.getSubscriptionsByPaymentMethod(paymentMethod.id).size,
                            onClick = {
                                ...
                                if (previousRoute == "addSub" ||
                                    ...
                                    viewModel.updateSelectedPaymentMethod(paymentMethod.id)
                                    navController.popBackStack()
                                }
                            },
                            onDeleteClick = {viewModel.removePaymentMethod(paymentMethod.id)}
                        )
                    }
                }
            }
            ...
            Button(
                onClick = {
                    viewModel.openPaymentMethodDialog()
                },
    ...
    if (viewModel.uiState.showDialogPaymentMethod) {
        AddPaymentMethodDialog(
            value = viewModel.uiState.namePaymentMethod,
            onValueChange = { viewModel.updateNamePaymentMethod(it) },
            onClick = {
                viewModel.savePaymentMethod(viewModel.uiState.namePaymentMethod)
...
private fun CategoryItemPreview() {
    BarbershopTheme {
        PaymentMethodItem("Карта 0023", 2, {}, {})
    }
}